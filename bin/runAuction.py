...
dir_simulations = os.path.abspath('..') + '/simulations/'
random.seed(seed)
...
def buildObjects(nettopObj): 	
	...
	Objects.indPriceDict = {f: (1000, 100) for f in Objects.federatenames}
...
def runAuctionProc(baseObj, nettopObj, fedBidDictList, proc):
	...
	for k, fedBidDict in enumerate(fedBidDictList):
		# try:
			...
			# print(solutionObj.independentValues, solutionObj.centrlaizedValues)
...
def multiprocrunAuction(nproc): 
	...
	# filenamelist = ['hashNetworkDict_elements10_federates2_density7', 'hashNetworkDict_elements15_federates3_density7']
